...
2. 간결한 프로젝트 인벤토리를 만든다.
   ...
   - 페이지, API, 기능 단위 entrypoint를 식별한다.
   - DB 접근 흔적을 찾는다. 예: raw SQL, stored procedure, ORM entity, migration, mapper XML, DAO/repository, DB connection config, query file.
...
3. readiness 산출물을 만들거나 갱신한다.
   ...
   - 프로젝트 구조와 핵심 파일 역할을 문서화한다.
   - 전체 프로젝트와 페이지/API 단위의 AI Agent Coding 준비도를 문서화한다.
   - DB 접근이 감지된 경우에만 `docs/db/` 산출물을 만든다.

4. DB 지식이 부족하면 사용자에게 요청한다.
   - stored procedure 호출, table 이름, query 흔적은 있지만 DDL, procedure definition, relationship, 상태값 의미를 알 수 없으면 추측하지 않는다.
   - 필요한 정보 목록을 사용자에게 질문한다.
   - 사용자가 즉시 제공하지 못하면 `docs/db/required-knowledge.md`에 요청 목록과 이유를 남긴다.
   - 사용자가 DDL 또는 stored procedure definition을 제공하면 `docs/db/`에 정리하고 readiness를 재평가한다.
...
7. `docs/project-structure.md` - 디렉터리 구조, 주요 모듈, generated/vendor/build output 구분.
8. `docs/file-roles.md` - 핵심 파일별 역할, entrypoint, config, routing, service/controller/model 파일.
9. `docs/development.md` - setup, 환경 변수, 로컬 작업 흐름.
10. `docs/testing.md` - 테스트 전략, 명령, gap.
11. `docs/agent-workflows.md` - 반복되는 AI Agent 작업 절차와 체크리스트.
12. `docs/readiness/project-readiness.md` - 전체 프로젝트의 AI Agent Coding 준비도.
13. `docs/readiness/pages/` 또는 `docs/readiness/apis/` - 페이지/API 단위 AI Agent Coding 준비도.
14. DB 접근이 감지된 경우에만 `docs/db/` - DB 지식, 부족한 DDL/procedure 요청, 관계, 위험.
15. 요청 시 오픈소스 파일: `CONTRIBUTING.md`, `SECURITY.md`, `LICENSE`, issue template, PR template, CI.

## 번들 리소스

- 무엇을 조사하고 어떤 gap을 남길지 판단할 때 `references/readiness-checklist.md`를 읽는다.
- 발견 내용을 어느 산출물에 넣을지 정할 때 `references/output-map.md`를 읽는다.
- AI Agent Coding 준비도를 산정할 때 `references/readiness-scoring.md`를 읽는다.
- DB 접근이 감지되거나 DB 지식이 부족하면 `references/db-knowledge.md`를 읽는다.
...
- DB schema, stored procedure, relationship, 상태값 의미는 추측하지 않는다. 모르면 사용자에게 요청하고 `docs/db/required-knowledge.md`에 남긴다.
- `docs/db/`는 DB 접근이 감지된 프로젝트에서만 생성한다.
- "전환률"보다 "AI Agent Coding 준비도"라는 명칭을 사용한다.
