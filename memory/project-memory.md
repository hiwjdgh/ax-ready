...
- 사용자 프로젝트 산출물에는 프로젝트 구조, 파일 역할, 전체/페이지/API 단위 AI Agent Coding 준비도 문서가 포함되어야 한다.
- DB 접근이 감지된 사용자 프로젝트에서만 `docs/db/`를 생성한다.
- DB DDL, stored procedure definition, relationship, status/code 의미가 부족하면 추측하지 않고 사용자에게 요청한다.
...
- `assets/templates/docs/readiness/`는 사용자 프로젝트의 AI Agent Coding 준비도 산출물 템플릿이다.
- `assets/templates/docs/db/`는 DB 접근 감지 시에만 사용자 프로젝트에 적용하는 템플릿이다.
...
- npm provenance 검증을 위해 `package.json`의 repository URL은 `https://github.com/hiwjdgh/ax-ready`와 일치해야 한다.
...
- release는 `.github/workflows/release.yml`을 수동 실행해 자동화한다. 이 workflow가 `npm version`, commit, tag, GitHub Release 생성, npm publish를 처리한다.
...
- GitHub Actions의 `GITHUB_TOKEN`으로 생성된 release 이벤트는 다른 workflow를 자동 트리거하지 않으므로, 기본 publish는 `release.yml` 안에서 수행한다.
...
- AI Agent Coding 준비도 산정 방식이 실제 프로젝트에서 과도하게 주관적이지 않은지 검증한다.
